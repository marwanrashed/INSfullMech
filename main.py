...
wz_bias = -0.001565 #np.mean(wz[0:bias_time])
fx_bias = 0.0725  #np.mean(fx[0:bias_time])
fy_bias = 0.2258 #np.mean(fy[0:bias_time])
# fz_bias = np.mean(fz[0:bias_time])

# wx -= wx_bias
# wy -= wy_bias
wz = list(map(lambda x : x- wz_bias , wz )) 
fx = list(map(lambda x : x- fx_bias  , fx )) 
fy = list (map(lambda x : x- fy_bias , fy )) 
# fx = fx.apply(lambda x : x- fx_bias ) 
# fy = fy.apply(lambda x : x- fy_bias ) 
# fz -= fz_bias
...
for i in range (start_time+1 , duration_2 ): 
    ...
    curr_latitude, curr_longitude, curr_altitude , curr_ve ,curr_vn, curr_vu, curr_roll, curr_pitch, curr_azimuth = INS_Mechanize.compile (0.2, 
                wx[i], wy[i], wz[i]
                , fx[i], fy[i],fz[i] ,latitude_prev,longitude_prev,altitude_prev,ve_Prev, vn_Prev, vu_Prev,g ) 
